      ...
      <div style={{ fontFamily: "'Sora', sans-serif", background: "#fff", color: "#000", minHeight: "100vh" }}>
          ...
          <div style={{ position: "fixed", inset: 0, zIndex: 500 }} onClick={() => setCartOpen(false)}>
            ...
            >
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "28px 32px 20px", borderBottom: "1px solid #f0f0f0" }}>
                <div>
                  <div style={{ fontSize: 20, fontWeight: 700, letterSpacing: "-0.03em" }}>Cart</div>
                  {cartCount > 0 && <div style={{ fontSize: 12, color: "#888", marginTop: 2 }}>{cartCount} item{cartCount !== 1 ? "s" : ""}</div>}
                </div>
                <button onClick={() => setCartOpen(false)} className="nav-btn" style={{ background: "#f5f5f5", border: "none", width: 36, height: 36, borderRadius: "50%", cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center", color: "#000" }}>✕</button>
              </div>

              {!cart || cart.lines.length === 0 ? (
                <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, color: "#999" }}>
                  <div style={{ fontSize: 40 }}>🛒</div>
                  <div style={{ fontSize: 15, fontWeight: 500, color: "#000" }}>Your cart is empty</div>
                  <div style={{ fontSize: 13 }}>Add some gear to get started</div>
                </div>
              ) : (
                <>
                  <div style={{ flex: 1, overflowY: "auto", padding: "20px 32px" }}>
                    {cart.lines.map((line) => (
                      <div key={line.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 0", borderBottom: "1px solid #f5f5f5" }}>
                        <div>
                          <div style={{ fontSize: 14, fontWeight: 600, letterSpacing: "-0.01em", marginBottom: 4 }}>{line.merchandise.product.title}</div>
                          <div style={{ fontSize: 12, color: "#888" }}>Qty {line.quantity}</div>
                        </div>
                        <div style={{ fontSize: 15, fontWeight: 700 }}>${(parseFloat(line.merchandise.priceV2.amount) * line.quantity).toFixed(2)}</div>
                      </div>
                    ))}
                  </div>
                  <div style={{ padding: "20px 32px 32px", borderTop: "1px solid #f0f0f0" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
                      ...
                      <span style={{ fontSize: 20, fontWeight: 700, letterSpacing: "-0.03em" }}>${cartTotal.toFixed(2)}</span>
                    </div>
                    ...
                    <div style={{ textAlign: "center", marginTop: 12, fontSize: 11, color: "#aaa" }}>Free shipping on orders over $35</div>
                  </div>
                </>
              )}
            </div>
        ...
        <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 200, height: 56, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 48px", background: scrolled ? "rgba(255,255,255,0.88)" : "rgba(255,255,255,0.72)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderBottom: scrolled ? "1px solid rgba(0,0,0,0.08)" : "1px solid transparent", transition: "all 0.3s ease" }}>
          <a href="#" style={{ fontFamily: "'Sora', sans-serif", fontSize: 17, fontWeight: 700, color: "#000", textDecoration: "none", letterSpacing: "-0.04em" }}>
            eschool<span style={{ color: "#555" }}>deals</span>
          </a>
          <div style={{ display: "none", gap: 32, position: "absolute", left: "50%", transform: "translateX(-50%)" }}>
            {["Products", "About", "Support"].map((item) => (
              <a key={item} href="#" style={{ fontSize: 13, fontWeight: 500, color: "#000", textDecoration: "none", opacity: 0.7 }}>{item}</a>
            ))}
          </div>
          <button onClick={() => setCartOpen(true)} className="nav-btn" style={{ display: "flex", alignItems: "center", gap: 8, background: "none", border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#000", padding: "8px 16px", borderRadius: 100, background: "#f0f0f0" }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>
            Bag
            {cartCount > 0 && <span style={{ background: "#000", color: "#fff", fontSize: 9, fontWeight: 700, minWidth: 16, height: 16, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 4px" }}>{cartCount}</span>}
          </button>
        </nav>
        ...
        <section style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "120px 32px 80px", background: "linear-gradient(180deg, #fafafa 0%, #fff 100%)", position: "relative", overflow: "hidden" }}>
          ...
          <h1 className="hero-h1" style={{ fontSize: "clamp(48px, 7vw, 96px)", fontWeight: 800, letterSpacing: "-0.05em", lineHeight: 0.96, marginBottom: 28, maxWidth: 760 }}>
            Everything<br />
            <span style={{ color: "#999" }}>you actually need.</span>
          </h1>
        ...
        <section id="products" style={{ padding: "100px 48px" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 48, flexWrap: "wrap", gap: 24 }}>
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#aaa", marginBottom: 8 }}>The Collection</div>
                <h2 style={{ fontSize: "clamp(28px, 3vw, 44px)", fontWeight: 800, letterSpacing: "-0.04em", lineHeight: 1 }}>Our picks for students.</h2>
              </div>
                    ...
                    >
                      ...
                      <div style={{ aspectRatio: "1", display: "flex", alignItems: "center", justifyContent: "center", background: "#f5f5f5", position: "relative", overflow: "hidden" }}>
                        {p.image
                          ...
                          : <span style={{ fontSize: 80, filter: "grayscale(20%)", transition: "transform 0.4s ease", transform: hovered === p.id ? "scale(1.08)" : "scale(1)" }}>{p.emoji}</span>
                        }
                      </div>

                      {/* Info */}
                      <div style={{ padding: "20px 20px 20px" }}>
                        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#aaa", marginBottom: 6 }}>{p.cat}</div>
                        <div style={{ fontSize: 16, fontWeight: 700, letterSpacing: "-0.02em", lineHeight: 1.2, marginBottom: 6 }}>{p.name}</div>
                        {p.desc && <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: "#999", fontWeight: 300, marginBottom: 16, lineHeight: 1.5 }}>{p.desc}</div>}
                        ...
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: p.desc ? 0 : 16 }}>
                          <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
                            ...
                            <span style={{ fontSize: 20, fontWeight: 700, letterSpacing: "-0.03em" }}>${p.price}</span>
                          </div>
                          <button
                            className="add-btn"
                            onClick={() => handleAdd(p)}
        ...
        <section style={{ background: "#000", color: "#fff", padding: "80px 48px" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 48 }}>
            {[
              ...
              <div key={item.title}>
                <div style={{ fontSize: 28, marginBottom: 16, opacity: 0.4 }}>{item.icon}</div>
                <div style={{ fontSize: 16, fontWeight: 700, letterSpacing: "-0.02em", marginBottom: 8 }}>{item.title}</div>
                <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "rgba(255,255,255,0.45)", lineHeight: 1.6, fontWeight: 300 }}>{item.body}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section style={{ padding: "100px 48px", textAlign: "center", background: "#fafafa" }}>
          <div style={{ maxWidth: 540, margin: "0 auto" }}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#aaa", marginBottom: 20 }}>Stay in the loop</div>
            <h2 style={{ fontSize: "clamp(28px, 3.5vw, 44px)", fontWeight: 800, letterSpacing: "-0.04em", marginBottom: 16, lineHeight: 1.05 }}>Get deals before<br />everyone else.</h2>
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 15, color: "#888", marginBottom: 36, lineHeight: 1.65, fontWeight: 300 }}>New arrivals, restocks, and student promos — delivered to your inbox.</p>
            ...
            <div style={{ marginTop: 14, fontSize: 11, color: "#bbb" }}>No spam. Unsubscribe anytime.</div>
          </div>
        </section>
          ...
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 48, marginBottom: 64 }}>
              ...
              <div style={{ display: "flex", gap: 64, flexWrap: "wrap" }}>
                <div>
                  <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(255,255,255,0.3)", marginBottom: 16 }}>Products</div>
                  <ul style={{ listStyle: "none" }}>
                    {products.map((p) => <li key={p.id} style={{ marginBottom: 10 }}><a href="#products" style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "rgba(255,255,255,0.45)", textDecoration: "none", fontWeight: 300 }}>{p.name}</a></li>)}
                  </ul>
                </div>
                <div>
                  <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(255,255,255,0.3)", marginBottom: 16 }}>Help</div>
                  <ul style={{ listStyle: "none" }}>
                    {["Track Order", "Returns", "Student Discount", "Contact", "FAQ"].map((l) => (
                      <li key={l} style={{ marginBottom: 10 }}><a href="#" style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "rgba(255,255,255,0.45)", textDecoration: "none", fontWeight: 300 }}>{l}</a></li>
                    ))}
                  </ul>
                </div>
              </div>
            ...
            <div style={{ borderTop: "1px solid rgba(255,255,255,0.07)", paddingTop: 24, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
              <span style={{ fontSize: 11, color: "rgba(255,255,255,0.25)" }}>© 2025 eschooldeals</span>
              <div style={{ display: "flex", gap: 24 }}>
                {["Privacy", "Terms"].map((l) => <a key={l} href="#" style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", textDecoration: "none" }}>{l}</a>)}
              </div>
            </div>
          </div>
        </footer>
