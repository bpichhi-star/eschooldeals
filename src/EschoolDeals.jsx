...
export default function EschoolDeals() {
  ...
  return (
        <div style={s.root}>
                <nav style={s.nav}>
                          <a href="#" style={s.logo}>eschool<span style={s.accent}>deals</span>span></a>a>
                          <div style={s.navRight}>
                                      <button style={s.navIcon}>
                                                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>svg>
                                      </button>button>
                                    <a href="#" style={s.cartPill}>
                                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>svg>
                                                Cart <span style={s.cartBadge}>{cartCount}</span>span>
                                    </a>a>
                          </div>div>
                </nav>nav>
              ...
              <section style={s.hero}>
                      <div style={s.eyebrow}>
                                <svg width="8" height="8" viewBox="0 0 8 8"><circle cx="4" cy="4" r="4" fill="currentColor"/></svg>svg>
                                Student Essentials
                      </div>div>
                      <h1 style={s.h1}>Everything You Need,<br/><span style={s.accent}>Nothing You Don't.</span>span></h1>h1>
                      <p style={s.heroSub}>Five carefully picked products for students. Quality gear at prices that don't hurt.</p>p>
                      <div style={s.heroBtns}>
                                <a href="#products" style={s.btnPrimary}>Shop Now</a>a>
                                <a href="#" style={s.btnSecondary}>Student Discount</a>a>
                      </div>div>
                      <div style={s.strip}>
                                <span style={s.stripItem}>📦 Free shipping $35+</span>span>
                                <div style={s.stripDot}/>
                                <span style={s.stripItem}>🎓 10% student discount</span>span>
                                <div style={s.stripDot}/>
                                <span style={s.stripItem}>🔄 30-day returns</span>span>
                      </div>div>
              </section>section>
        
              <section style={s.productsSec} id="products">
                      <div style={s.secLabel}>Our Range</div>div>
                      <div style={s.secTitle}>The Full Collection</div>div>
                      <div style={s.grid}>
                        {products.map(p => (
                      <div key={p.id} style={s.card}>
                        {p.badge && <span style={{...s.badge,...(p.badge==="new"?s.badgeNew:s.badgePop)}}>{p.badge==="new"?"New":"Popular"}</span>span>}
                                    <button style={s.fav} onClick={()=>setFavs(f=>({...f,[p.id]:!f[p.id]}))}>
                                      {favs[p.id]?"♥":"♡"}
                                    </button>button>
                                    <div style={s.cardImg}>{p.emoji}</div>div>
                                    <div style={s.cardBody}>
                                                    <div style={s.cardCat}>{p.cat}</div>div>
                                                    <div style={s.cardName}>{p.name}</div>div>
                                                    <div style={s.cardBottom}>
                                                                      <div>
                                                                        {p.oldPrice && <span style={s.oldPrice}>${p.oldPrice}</span>span>}
                                                                                          <span style={s.price}>${p.price}</span>span>
                                                                      </div>div>
          ...
          {icon:"🔄",title:"Easy Returns",sub:"30-day hassle-free returns"}
                              ...
                              <div key={item.title} style={{...s.infoItem,...(i===2?{borderRight:"none"}:{})}}>
                                          <span style={s.infoIcon}>{item.icon}</span>span>
                                          <div><div style={s.infoTitle}>{item.title}</div>div><div style={s.infoSub}>{item.sub}</div>div></div>div>
                              </div>div>
                            ))}
              </div>div>
        
              <section style={s.newsletter}>
                      <h2 style={s.nlH2}>Get <span style={s.accent}>student deals</span>span> in your inbox</h2>h2>
                      <p style={s.nlSub}>No spam. Just deals, restocks, and new arrivals.</p>p>
                      <div style={s.nlForm}>
                                <input style={s.nlInput} type="email" placeholder="your@email.com" value={email} onChange={e=>setEmail(e.target.value)}/>
                                <button style={s.btnPrimary} onClick={()=>setEmail("")}>Subscribe</button>button>
                      </div>div>
              </section>section>
              ...
              <footer style={s.footer}>
                      <div style={s.footerInner}>
                                <div>
                                            <a href="#" style={{...s.logo,color:"#F8F7F4",fontSize:18}}>eschool<span style={s.accent}>deals</span>span></a>a>
                                            <p style={s.footerTagline}>Simple gear for students. No fluff, no markup.</p>p>
                                </div>div>
                                <div>
                                            <div style={s.footerColTitle}>Products</div>div>
                                            <ul style={s.footerList}>{products.map(p=><li key={p.id}><a href="#" style={s.footerLink}>{p.name}</a>a></li>li>)}</ul>ul>
                                </div>div>
                                <div>
                                            <div style={s.footerColTitle}>Help</div>div>
                                            <ul style={s.footerList}>{["Track My Order","Returns","Student Discount","Contact Us","FAQ"].map(l=><li key={l}><a href="#" style={s.footerLink}>{l}</a>a></li>li>)}</ul>ul>
                                </div>div>
                      </div>div>
                      <div style={s.footerBottom}>
                                <span>© 2025 eschooldeals. All rights reserved.</span>span>
                                <div style={{display:"flex",gap:20}}>
                                            <a href="#" style={s.footerBottomLink}>Privacy</a>a>
                                            <a href="#" style={s.footerBottomLink}>Terms</a>a>
                                </div>div>
                      </div>div>
              </footer>footer>
        </div>div>
