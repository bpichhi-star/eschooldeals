...
import { fetchRssDeals }     from '@/lib/feeds/rss'
...
export async function runIngest({ triggerType = 'cron' } = {}) {
  ...
  const [walmartRes, wootSerpRes, rssRes] = await Promise.allSettled([
    fetchWalmartDeals(),
    fetchWootSerp(),
    fetchRssDeals(),
  ])
  ...
  if (rssRes.status      === 'rejected') console.error(`[ingest:${triggerType}] rss failed:`,       rssRes.reason)

  console.log(`[ingest:${triggerType}] fetched — walmart: ${walmart.length}, woot-serp: ${wootSerp.length}, rss: ${rss.length}`)

  // 3. Upsert as pending
  const allDeals = [...walmart, ...wootSerp, ...rss]
  ...
  return {
    ...
    rss_failed:       rssRes.status      === 'rejected',
  }
}
