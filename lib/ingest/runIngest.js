...
export const ALL_SOURCES = ['walmart', 'target', 'slickdeals']
...
export async function runIngest({ triggerType = 'cron', sources = ALL_SOURCES } = {}) {
  ...
  const [walmartRes, targetRes, slickdealsRes] = await Promise.allSettled([
  ...
  console.log('[ingest:' + triggerType + '] fetched — walmart: ' + walmart.length + ', target: ' + target.length + ', slickdeals: ' + slickdeals.length)

  // ── Step 4: Upsert new deals ─────────────────────────────────────────────────
  const allDeals = [...walmart, ...target, ...slickdeals]
