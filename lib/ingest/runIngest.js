...
export const ALL_SOURCES = ['walmart', 'target', 'slickdeals', 'dealnews']
...
export async function runIngest({ triggerType = 'cron', sources = ALL_SOURCES } = {}) {
  ...
  const [walmartRes, targetRes, slickdealsRes, dealnewsRes] = await Promise.allSettled([
  ...
  console.log('[ingest:' + triggerType + '] fetched — walmart: ' + walmart.length + ', target: ' + target.length + ', slickdeals: ' + slickdeals.length + ', dealnews: ' + dealnews.length)

  // ── Step 4: Upsert new deals ─────────────────────────────────────────────────
  const allDeals = [...walmart, ...target, ...slickdeals, ...dealnews]
  ...
  return {
    ...
    slickdeals: slickdeals.length, dealnews: dealnews.length,
