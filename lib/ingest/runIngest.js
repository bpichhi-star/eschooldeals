...
export async function runIngest({ triggerType = 'cron' } = {}) {
  ...
  if (!supabase) {
    throw new Error('Supabase admin client not initialized — check env vars')
  }
  ...
  if (walmartRes.status === 'rejected')
    console.error(`[ingest:${triggerType}] walmart failed:`, walmartRes.reason?.message)
  if (slickdealsRes.status === 'rejected')
    console.error(`[ingest:${triggerType}] slickdeals failed:`, slickdealsRes.reason?.message)
  if (edealinfoRes.status === 'rejected')
    console.error(`[ingest:${triggerType}] edealinfo failed:`, edealinfoRes.reason?.message)

  console.log(`[ingest:${triggerType}] fetched — walmart: ${walmart.length}, slickdeals: ${slickdeals.length}, edealinfo: ${edealinfo.length}`)

  // 3. Upsert each source
  const allDeals = [...walmart, ...slickdeals, ...edealinfo]
