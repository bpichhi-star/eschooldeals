unction mapFeedItem(item, provider) {
  return {
...
export async function runFeedAdapter(provider) {
    const config = provider?.config || {}
        ...
        // Mode: mockItems — use static data directly (no scraping needed)
        if (
              (config.mode === 'mockItems' || config.mode === 'amazon_asins') &&
              Array.isArray(config.mockItems) &&
              config.mockItems.length > 0
            ) {
              return config.mockItems
                .map((item) => mapFeedItem(item, provider))
