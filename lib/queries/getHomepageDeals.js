...
function isStudentRelevant(deal) {
  // Admin-added deals always show — admin's judgment overrides automated filters.
  // Only sanity-check that it has a price.
  if (deal.source_key === 'manual') {
...
export async function getHomepageDeals(limit = 2000) {
  ...
  // Fetch deals matching one of the 7 student-facing categories OR any admin-added
  // (manual) deal regardless of category. The .or() filter combines both criteria.
  const categoryFilter = STUDENT_CATEGORIES.map(c => '"' + c + '"').join(',')
  const { data, error } = await supabase
    .from('deals')
    .select('*')
    .eq('status', 'active')
    .or('category.in.(' + categoryFilter + '),source_key.eq.manual')
    .gt('sale_price', 0)
