...
// Hard blocklist — titles containing these patterns never show on the homepage,
// regardless of category. Catches pet, baby, automotive, and office supply
// items that slip through with vague categories.
//
// Note: bare 'cat' was previously in this list and false-matched 'Cat-6'
// networking cables. Replaced with specific cat-product phrases below so
// networking gear stays visible. Same surgical-phrase approach could be
// applied to 'dog' / 'pet' if future false positives surface.
const TITLE_BLOCKLIST = /\b(dog|cat food|cat litter|cat toy|cat tree|cat scratcher|cat carrier|cat collar|cat bed|cat harness|pet|puppy|kitten|bird|fish tank|aquarium|hamster|rabbit|guinea pig|baby|infant|toddler|diaper|pacifier|stroller|crib|car seat|automotive|oil filter|wiper blade|car part|tire|brake|caster|chair wheel|office chair wheel|furniture leg|desk leg|table leg|cabinet hinge|door hinge|plumbing|toilet|faucet|sink|bathtub|lawn|garden|fertilizer|mulch|weed|pesticide|insecticide|hunting|ammo|firearm|gun|knife blade|tactical)\b/i
...
function isStudentRelevant(deal) {
  ...
  // No price ceiling — show high-priced items too. Original ceilings (Home $500,
  // Accessories $300, Electronics $1500, etc.) were filtering out legit items
  // like kitchen appliances, audio gear, and high-end cables. Title blocklist
  // still applies.
