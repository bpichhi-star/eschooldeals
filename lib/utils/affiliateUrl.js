...
// CJ deep-linking only accepts URLs on the advertiser's whitelisted domain.
// For Woot, that's www.woot.com — clicks with a category subdomain in the
// `?url=` parameter (electronics.woot.com, sellout.woot.com, computers.woot.com,
// etc.) are rejected by CJ and bounce to members.cj.com/member/404.html.
// Woot's canonical product URL pattern is www.woot.com/offers/{slug}, and
// the category subdomains serve the same offers via alias, so this rewrite
// is safe for any /offers/, /plus/, /category/, or /alldeals path.
function normalizeWootHost(url = '') {
  return url.replace(/^https?:\/\/(?:[a-z0-9-]+\.)?woot\.com/i, 'https://www.woot.com')
}

export function buildAffiliateUrl(rawUrl, asin = null) {
  if (!rawUrl) return rawUrl

  // Already wrapped in CJ — check if the inner destination needs Woot
  // subdomain normalization (self-heals existing broken wraps when re-saved
  // through admin/edit). For non-Woot wrapped URLs, leave alone.
  if (isAlreadyCJWrapped(rawUrl)) {
    try {
      const u = new URL(rawUrl)
      const inner = u.searchParams.get('url')
      if (inner && /woot\.com/i.test(inner) && !/^https?:\/\/www\.woot\.com/i.test(inner)) {
        u.searchParams.set('url', normalizeWootHost(inner))
        return u.toString()
      }
    } catch { /* fall through */ }
    return rawUrl
  ...
  // ── Woot: normalize subdomain → www, then wrap in CJ deep-link ─────────
  if (vendor === 'woot') {
    const clean = normalizeWootHost(cleanUrl(rawUrl))
