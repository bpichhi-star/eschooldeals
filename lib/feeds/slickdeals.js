...
// Resolver tuning. Constraints to satisfy:
//   - Vercel function budget = 60s (maxDuration in the cron route)
//   - ~100 items typically need HTTP resolution (the other ~30% are caught
//     by free ASIN extraction with no network call)
//   - Per-request timeout vs concurrency tradeoff:
//       8s timeout, 16-wide concurrency → 100/16 = 7 batches × 8s = 56s
//       4s timeout,  8-wide concurrency → 100/8  = 13 batches × 4s = 52s
//
// Bumped from 4s/8-wide → 8s/16-wide on April 29 2026: gives slow
// merchant click handlers (long-tail Awin sub-trackers, Sovrn) more
// time to respond. Typical click handlers respond in 100-500ms so
// realistic ingest time is ~3-5s; the worst case only kicks in if
// we hit dozens of slow ones in a row.
