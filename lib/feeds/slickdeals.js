...
// champssports.4xc4ep.net for Awin sub-tracker).
const AFFILIATE_REDIRECT_HOSTS = new Set([
  'awin1.com', '4xc4ep.net',                              // Awin
  'sjv.io', 'igs4ds.net', 'pntra.com', 'pntrac.com',      // Sovrn / Partnerize
  'go.redirectingat.com', 'redirectingat.com',            // Skimlinks
  'anrdoezrs.net', 'dpbolvw.net', 'jdoqocy.com',          // CJ Affiliate
  'kqzyfj.com', 'tkqlhce.com',                            // CJ alternates
  'rover.ebay.com', 'shopstyle.it',                       // Misc
  'goto.walmart.com', 'goto.target.com',                  // Walmart/Target Impact
...
// If `url` is an affiliate-redirect (Awin/Sovrn/Skimlinks/CJ/etc.), pull the
// real merchant destination out of its query params. Returns null if not on
// a known affiliate host or no destination param is present.
...
async function parseItem(itemXml, feedLabel) {
  ...
  if (TITLE_BLOCKLIST.test(title)) return null
  if (VAGUE_TITLE_RE.test(title))  return null
  ...
  if (asin) {
    ...
    if (isAffiliateRedirect(resolved)) return null  // still wrapped — give up
