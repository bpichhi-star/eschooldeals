...
function mapRedskyProduct(item, queryConfig) {
  ...
  return {
    ...
    score:               Math.min(discount_pct, 50)
                       + 10
                       + (reviews >= 100 ? 10 : reviews >= 25 ? 5 : 0)
                       + (queryConfig.student ? 5 : 0),
...
export async function fetchTargetDeals() {
  ...
  console.log('[target] Fetching ' + queries.length + ' queries (seasonal=' + isBackToSchoolSeason() + ')')

  const results = await Promise.allSettled(queries.map(fetchTargetQuery))
