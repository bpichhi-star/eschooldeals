...
// ─── Helpers ──────────────────────────────────────────────────────────────────
// SerpApi Target sometimes returns price as a string like "$29.99" — strip non-numeric chars
...
async function searchTarget({ q, category, max_price, student }) {
  ...
  return items
    .filter(item => {
      const price = parsePriceStr(item.price?.current ?? item.current_price ?? item.price)
    ...
    .map(item => {
      const sale_price     = parsePriceStr(item.price?.current ?? item.current_price ?? item.price)
