...
// ─── Main entry point ─────────────────────────────────────────────────────────
// Best Buy enforces a per-second rate limit on free tier — running calls in
// parallel reliably trips 403s. Serialize calls with a small delay between them.
const RATE_LIMIT_DELAY_MS = 1100  // ~1 req/sec, safe under BB's per-second cap
...
export async function fetchBestBuyDeals() {
  ...
  console.log('[bestbuy] Fetching ' + SALE_CATEGORIES.length + ' sale + ' + OPEN_BOX_CATEGORIES.length + ' open-box categories (serialized, ' + RATE_LIMIT_DELAY_MS + 'ms apart)')

  const saleResults    = await runSerialWithDelay(SALE_CATEGORIES, fetchSaleByCategory, RATE_LIMIT_DELAY_MS)
  const openBoxResults = await runSerialWithDelay(OPEN_BOX_CATEGORIES, fetchOpenBoxByCategory, RATE_LIMIT_DELAY_MS)
