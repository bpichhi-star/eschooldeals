...
const SEARCH_QUERIES = [
  { query: 'laptop deal clearance',    category: 'Computers',   studentBonus: true  },
...
async function searchWalmart({ query, category, studentBonus }) {
  const params = new URLSearchParams({ engine: 'walmart', query, api_key: SERPAPI_KEY, sort_by: 'best_seller' })
  const res = await fetch(`https://serpapi.com/search.json?${params}`)
  if (!res.ok) { console.error(`[walmart] SerpApi ${res.status} for "${query}"`); return [] }
  const data = await res.json()
  return (data.organic_results || [])
    .filter(item => parseFloat(item.primary_offer?.offer_price) > 0 && item.thumbnail && item.product_page_url)
    .slice(0, 10)
    .map(item => {
      const sale_price = parseFloat(item.primary_offer?.offer_price)
      if (!sale_price || sale_price <= 0) return null
      // Check all SerpApi fields where strikethrough price might appear
      const rawWas =
        item.was_price                      ??
        ...
        item.list_price                     ??
        null
      const parsed_was     = rawWas != null ? parseFloat(rawWas) : null
      const original_price = (parsed_was && parsed_was > sale_price) ? parsed_was : null
...
export async function fetchWalmartDeals() {
  if (!SERPAPI_KEY) { console.warn('[walmart] SERPAPI_KEY not set — skipping'); return [] }
  console.log('[walmart] Fetching', SEARCH_QUERIES.length, 'queries')
  const results = await Promise.allSettled(SEARCH_QUERIES.map(q => searchWalmart(q)))
  ...
  console.log(`[walmart] ${unique.length} unique deals`)
  return unique
}
