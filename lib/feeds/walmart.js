...
const QUERIES = [
  ...
  // School supplies
  { q: 'composition notebook 10 pack',           category: 'Accessories',     max_price: 20,   student: true  },
  // More tech
  ...
  { q: 'running shoes men women under 60',        category: 'Sports',     max_price: 65,   student: true  }
]
...
async function searchWalmart({ q, category, max_price, student }) {
  ...
  return items
    ...
    .map(item => {
      ...
      // Prefer title-based categorization (centralized rules); fall back to
      // the query config's category only if categorize() can't decide.
      const titleCategory = categorize(item.title || '')
      const finalCategory = titleCategory !== 'General' ? titleCategory : category
