...
// Sort options for the homepage grid. Default is 'latest'.
//
//   latest      → newest first, by created_at DESC. Matches server default.
...
//   clicks      → most clicked first (uses deals.click_count populated by
//                 /api/track-click). Cold-start tiebreaker is created_at.
//
// Sorts are applied to the FILTERED list, so they always reflect what the
// user is currently viewing (category tab + search). Sorting 600 items
...
function sortDeals(list, sortBy) {
  ...
  switch (sortBy) {
    ...
    case 'discount':
      ...
      break
    case 'savings': {
      // Compute on the fly (we don't store savings in the DB; it's
      // derived). Tiebreak on discount % so two items with the same
      // dollar savings show the more aggressive % off first.
    ...
    case 'clicks':
      ...
      // Tiebreak by created_at DESC so newer no-click items beat older.
      sorted.sort((a, b) => {
